...
class FcbCli:
    ...
    def run_sim(self, flight_filepath: str) -> None:
        ...
        for row in df.itertuples():
            ...
            data = struct.pack(
                ...
                -1 * row.imu1_accel_y,
                -1 * row.imu1_accel_z,
                -1 * row.imu1_accel_x_real,
                -1 * row.imu1_accel_y_real,
                -1 * row.imu1_accel_z_real,
                -1 * row.imu1_gyro_x,
                -1 * row.imu1_gyro_y,
                -1 * row.imu1_gyro_z,
                -1 * row.imu1_gyro_x_real,
                -1 * row.imu1_gyro_y_real,
                -1 * row.imu1_gyro_z_real,
                row.imu1_mag_x,
                row.imu1_mag_y,
                row.imu1_mag_z,
                row.imu1_mag_x_real,
                ...
                row.gps_long,
                row.gps_alt,
                0 if "gps_fix_quality" not in df.columns else row.gps_fix_quality,
                0 if "gps_sats_tracked" not in df.columns else row.gps_sats_tracked,
                0 if "gps_hdop" not in df.columns else row.gps_hdop,
                0,
                0,
                0,
                0,
    ...
    def _sensor_data_struct_str(self) -> str:
        ...
        pack_properties = [
            ...
            PackProperty("gps_fix_quality", "B"),
            PackProperty("gps_sats_tracked", "B"),
            PackProperty("gps_hdop", "f"),
            ...
            PackProperty("battery_voltage_2", "d"),
            PackProperty("pyro_continuity", "????"),
            PackProperty("dynamixel_degrees", "d"),
            PackProperty("dynamixel_degrees_1", "d"),
            PackProperty("dynamixel_length", "d"),
            PackProperty("dynamixel_length_1", "d")
        ]
    ...
    def _log_data_struct(self) -> List[List[UnpackProperty]]:
        ...
        unpack_properties: List[List[UnpackProperty]] = [
            [  # FCB Data
                ...
                UnpackProperty("battery_voltage_1", "d"),
                UnpackProperty("battery_voltage_2", "d"),
                UnpackProperty("pyro_cont", "B"),
                UnpackProperty("dynamixel_degrees", "d"),
                UnpackProperty("dynamixel_degrees_1", "d"),
                UnpackProperty("dynamixel_length", "d"),
                UnpackProperty("dynamixel_length_1", "d"),

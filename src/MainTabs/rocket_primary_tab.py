...
class RocketPrimaryTab(TabCommon):
    def __init__(self, tab_name, parent=None):
        ...
        self.FlightDisplay = self.addWidget(
            flight_display.FlightDisplay(compass_and_text=False)
        )
        ...
        self.AltitudeGraph = self.addWidget(
            graph_widget.GraphWidget(
                title="Altitude", source_list=[Constants.altitude_key]
            )
        )

        self.ButtonPanel.clearMapButton.clicked.connect(
            self.Map.clearMap
        )  # This is a sketchy way to do this
        ...
        layout.addWidget(
            self.Annunciator, 4, 3, 1, 1
        )  # Lower right (but biased toward center)
    ...
    def customUpdateVehicleData(self, data):
        self.ButtonPanel.setMaximumWidth(
            max(self.FlightDisplay.width() - self.Annunciator.width() - 6, 5)
        )

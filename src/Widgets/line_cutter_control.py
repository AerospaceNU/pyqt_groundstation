...
class LineCutterControl(custom_q_widget_base.CustomQWidgetBase):
    ...
    def onCutButtonPressed(self, button_number):
        if self.cutter_enabled:
            self.callback_handler.requestCallback(Constants.cli_interface_key, '--linecutter -i {0} -c "!cut {1}"'.format(self.active_line_cutter, button_number))
    ...
    def setArmOrDisarm(self, armed):
        ...
        if self.cutter_armed:
            ...
            self.callback_handler.requestCallback(Constants.cli_interface_key, '--linecutter -i {0} -c "!arm"'.format(self.active_line_cutter))
        else:
            self.arm_button.setText("Arm Line Cutter")
            self.callback_handler.requestCallback(Constants.cli_interface_key, '--linecutter -i {0} -c "!disarm"'.format(self.active_line_cutter))

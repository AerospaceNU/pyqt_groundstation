...
class FCBDataInterfaceCore(ThreadedModuleCore):
    ...
    def handleParsedData(self, message_type, dictionary, update_on_bad_crc=True):
        ...
        if Constants.fcb_state_number_key in dictionary:
            dictionary[Constants.fcb_state_key] = get_fcb_state_from_state_num(
                dictionary[Constants.fcb_state_number_key]
            )

        self.diagnostics_box_helper.updatePanel(
            message_type, dictionary
        )  # Update the diagnostics box with the unfiltered data
        ...
        if self.good_radio_crc or update_on_bad_crc:
            ...
            if (
                Constants.latitude_key in dictionary
                and Constants.longitude_key in dictionary
            ):  # If dictionary contains vehicle gps position, filter it
                ...
                if Constants.ground_speed_key not in dictionary:
                    ...
                    ] = self.vehicle_position_filter.get_filtered_speed_output()

            # Filter ground station lat and lon
            if (
                Constants.ground_station_latitude_key in dictionary
                and Constants.ground_station_longitude_key in dictionary
            ):
            ...
            if Constants.fcb_state_number_key in dictionary:
                self.fcb_state_filter.new_data(
                    dictionary[Constants.fcb_state_number_key]
                )
                dictionary[Constants.fcb_state_number_key] = int(
                    self.fcb_state_filter.get_filtered_data_output()
                )
                dictionary[Constants.fcb_state_key] = get_fcb_state_from_state_num(
                    dictionary[Constants.fcb_state_number_key]
                )
    ...
    def updateEveryEnabledLoop(self):
        ...
        if self.connected:
            self.annunciator.setAnnunciator(
                0, "GS Connection", 0, "Connected to ground station"
            )
        else:
        ...
        if self.has_data:
            self.annunciator.setAnnunciator(
                1, "FCB Data", 0, "Receiving data from rocket"
            )
        else:
        ...
        if not self.good_fcb_data:
            ...
            self.annunciator.setAnnunciator(
                2, "Good FCB Data", 1, "Bad CRC on last packet"
            )
        else:
        ...
        self.data_dictionary[
            Constants.status_source
        ] = self.annunciator.getOverallStatus()
        self.data_dictionary[
            Constants.raw_message_data_key
        ] = self.diagnostics_box_helper.get_diagnostics_dict()

        self.data_dictionary[Constants.message_age_key] = round(
            time.time() - self.last_good_data_time, 3
        )

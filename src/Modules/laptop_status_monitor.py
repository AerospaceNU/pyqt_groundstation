...
class LaptopStatusMonitor(ThreadedModuleCore):
    ...
    def spin(self):
        ...
        self.data_dictionary[
            Constants.laptop_ram_usage_key
        ] = psutil.virtual_memory().percent
        self.data_dictionary[Constants.laptop_disk_usage_key] = psutil.disk_usage(
            "/"
        ).percent

        if battery is not None:
            self.data_dictionary[Constants.laptop_battery_percent_key] = float(
                battery.percent
            )
            self.data_dictionary[
                Constants.laptop_battery_charging_key

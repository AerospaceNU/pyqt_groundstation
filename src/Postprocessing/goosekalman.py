...
def calculate_altitude(pressure):
    ...
    while layer_number < NUMBER_OF_LAYERS - 2:
        ...
        if lapse_rate[layer_number] == 0.0:
            exponent = GRAVITATIONAL_ACCELERATION * delta_z / AIR_GAS_CONSTANT / base_temperature
            next_base_pressure = next_base_pressure * math.exp(exponent)

        else:
            base = (lapse_rate[layer_number] * delta_z / base_temperature) + 1.0
            exponent = GRAVITATIONAL_ACCELERATION / (AIR_GAS_CONSTANT * lapse_rate[layer_number])
    ...
    if lapse_rate[layer_number] == 0.0:
        coefficient = (AIR_GAS_CONSTANT / GRAVITATIONAL_ACCELERATION) * base_temperature
        altitude = base_altitude[layer_number] + coefficient * math.log(pressure / base_pressure)

    else:
        base = pressure / base_pressure
        exponent = AIR_GAS_CONSTANT * lapse_rate[layer_number] / GRAVITATIONAL_ACCELERATION
        coefficient = base_temperature / lapse_rate[layer_number]
        altitude = base_altitude[layer_number] + coefficient * (pow(base, exponent) - 1)
...
def make_fcb(num=3):
    ...
    if num == 1:
        ...
        df = pd.read_csv("~/Downloads/LDRS-beanboozler-l265-output-post.csv")
    elif num == 9:
        df = pd.read_csv("~/Downloads/2023-04-15-oorr-output-FCB-post.csv")
    ...
    altArr = np.array([calculate_altitude(p * 101325) for p in df["baro_pres_avg"]])

    altArr = altArr - altArr[0]
    csv["gps_alt"] = csv["gps_alt"] - csv["gps_alt"][0]

    dt = 0.0155  # On average, during flight
    # dt = .02 # In sim
...
def make_line_cutter():
    global timeArr, altArr, dt, ayArr
    df = pd.read_csv(
        "D:\\Documents\\GitHub\\dpf-line-cutter\\code\\launch archive\\2021-11-14 MDRA\\Cherry_flight_data.csv",
...
def rotate_accel():
    ...
    global stateArray
    global apogeeTime, csv

    import ahrs

    world_accel = np.zeros((csv.shape[0], 3))
    angle_vert = []
    for idx, row in csv.iterrows():
        ...
        accel_world = ahrs.common.orientation.q2R(quat) @ accel
        world_accel[idx, :] = accel_world.reshape((1, 3))

        angle_vert.append(math.degrees(math.acos((ahrs.common.orientation.q2R(quat) @ np.array([1, 0, 0])) @ np.array([0, 0, 1]))))

    world_accel = np.array(world_accel)
    print(world_accel)

    print(angle_vert[0])

    csv["world_accel_x"] = world_accel[:, 0]
    csv["world_accel_y"] = world_accel[:, 1]
    csv["world_accel_z"] = world_accel[:, 2]
...
def do_kalman():
    ...
    kalman_gain, P_steady = kalmd.kalmd(sysd, Q, R)
    ...
    csv_copy = csv.copy()
    csv_copy["posz_new"] = xhatarr[:, 0]
    csv_copy["velz_new"] = xhatarr[:, 1]
    csv_copy.to_csv("oorr_kalman_world.csv")

    plt.figure()
    plt.plot(timeArr, xhatarr[:, 1], label="Kalman velocity, m/s")
    plt.plot(timeArr, residual_arr, label="altitude residual, m")
    plt.legend()
    plt.figure()
    plt.subplot(211)
    plt.plot(timeArr, csv_copy["world_accel_x"], label="x")
    plt.plot(timeArr, csv_copy["world_accel_y"], label="y")
    plt.plot(timeArr, csv_copy["world_accel_z"], label="z")
    plt.plot(timeArr, csv_copy["imu1_accel_y_real"], label="board X")
    plt.legend()
    plt.subplot(212)
    plt.plot(timeArr, (csv_copy["imu1_accel_y_real"] - csv_copy["world_accel_z"]) / csv_copy["world_accel_z"] * 100, label="Delta, %")
    plt.legend()
    plt.show()
...
def do_gyro_integration(mag_data):
    ...
    if "wx_raw" in csv:
        ...
        gyro_data = np.array([-csv["imu1_gyro_x_real"], -csv["imu1_gyro_z_real"], csv["imu1_gyro_y_real"]]).T / 4  # /4 for sensitivity bug fix
    else:
    ...
    if "ax" in csv:
        acc_data = np.array([-csv["ax"] * 9.81, -csv["az"] * 9.81, csv["ay"] * 9.81]).T
    else:
        acc_data = np.array([-csv["imu1_accel_x_real"], csv["imu1_accel_z_real"], -csv["imu1_accel_y_real"]]).T
...
def rotation_matrix(quat):
    ...
    return np.array([[1 - (yy + zz), (xy - zw), (xz + yw)], [(xy + zw), 1 - (xx + zz), (yz - xw)], [(xz - yw), (yz + xw), 1 - (xx + yy)]])
...
def plot_kalman():
    ...
    # plt.plot(timeArr, csv["gps_alt"], label="GPS Position")

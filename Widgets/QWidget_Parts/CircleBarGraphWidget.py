...
class CircleBarGraphWidget(QLabel):
    ...
    def paintEvent(self, e):
        ...
        fontSize = max(int(self.width() * 0.1), 8)
        padding = 4
        ...
        smallFontSize = max(fontSize / 2, 6)
        painter.setFont(QFont("Monospace", smallFontSize))
        painter.drawText(QRect(0, (self.height() * 0.9 / 2) + smallFontSize + padding, self.width(), smallFontSize), Qt.AlignCenter, self.title)

        theta = clamp(interpolate(self.value, self.minValue, self.maxValue, 0, 360 * 16), 0, 360 * 16)
        barPadding = 5

        lineWidth = int(self.size / 7)
        painter.setPen(QPen(self.barColor, lineWidth, Qt.SolidLine))
